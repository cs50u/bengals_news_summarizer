...
def test_get_article_text_prefers_content_over_summary():
    ...
    text = bengals_digest.get_article_text(entry)
...
def test_fetch_bengals_articles_deduplicates(monkeypatch):
    ...
    monkeypatch.setattr(bengals_digest.feedparser, "parse", fake_parse)
    results = bengals_digest.fetch_bengals_articles(feeds, limit_per_feed=3, max_total=10)
...
def test_summarize_with_ollama_timeout(monkeypatch):
    ...
    def fake_run(*args, **kwargs):
        raise bengals_digest.subprocess.TimeoutExpired(cmd="ollama", timeout=1)
    monkeypatch.setattr(bengals_digest.subprocess, "run", fake_run)
    summary, elapsed, *_ = bengals_digest.summarize_with_ollama("Test")
...
def test_summarize_with_ollama_blanks(monkeypatch):
    ...
    monkeypatch.setattr(bengals_digest.subprocess, "run", lambda *a, **k: FakeResult(""))
    summary, *_ = bengals_digest.summarize_with_ollama("Test")
    ...
    monkeypatch.setattr(bengals_digest.subprocess, "run", lambda *a, **k: FakeResult("jeta not enough"))
    summary, *_ = bengals_digest.summarize_with_ollama("Test")
...
def test_integration_fake(monkeypatch):
    ...
    monkeypatch.setattr(bengals_digest, "fetch_bengals_articles", lambda feeds, limit_per_feed=5, max_total=12: fake_entries)
    monkeypatch.setattr(bengals_digest, "summarize_with_ollama", lambda text, model=None: (f"Summary for: {text[:12]}", 1, 0, 0, 0, 0))
    monkeypatch.setattr(bengals_digest, "get_article_text", lambda entry: entry.summary)

    # Patch out file writing (no actual I/O)
    monkeypatch.setattr(bengals_digest.os, "makedirs", lambda *a, **k: None)
    monkeypatch.setattr(bengals_digest, "open", lambda *a, **k: SimpleNamespace(write=lambda x: None).__enter__())
